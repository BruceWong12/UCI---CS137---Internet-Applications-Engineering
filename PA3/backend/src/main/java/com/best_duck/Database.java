...
public class Database {
    ...
    public static void setOrder(
        ...
        Date mysqlTodaysDate = new Date( todaysDate.getTime() );
    ...
    public static ArrayList<String> getStates(String name) {
        ...
        try {
            ...
            String sql;
            sql = "SELECT DISTINCT taxregionname FROM best_duck.tax_rates WHERE taxregionname LIKE \"%\"?\"%\"";
            ...
            while(rs.next()){
                list.add(rs.getString("taxregionname"));
            }
            rs.close();
            //stmt.close();
            conn.close();
    ...
    public static ArrayList<String> getStatesByName(String name) {
        ArrayList<String> list = new ArrayList<>();
        Connection conn = openConnection();
        System.out.println("Creating statement...");
        try {
            ...
            String sql;
            sql = "SELECT DISTINCT combinedrate FROM best_duck.tax_rates WHERE taxregionname=? limit 1";
            ...
            while(rs.next()){
                list.add(rs.getString("combinedrate"));
            }
            rs.close();
            //stmt.close();
            conn.close();
